...
class CSVDataHandler:
    ...
    def get_driver_standings(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for standing in standings_list:
                ...
                constructor = (
                    standing["Constructors"][0] if standing["Constructors"] else {}
                )
    ...
    def get_constructor_standings(self) -> pd.DataFrame:
        ...
        try:
            ...
            sorted_teams = sorted(
                team_data.values(), key=lambda x: x["Points"], reverse=True
            )
    ...
    def get_circuits(self) -> List[Dict[str, Any]]:
        ...
        circuits = df[
            ["circuitRef", "name_y", "location", "country", "lat", "lng"]
        ].drop_duplicates()
...
class ErgastAPI(CSVDataHandler):
    ...
    def __init__(self, cache_dir: str = None):  # pylint: disable=unused-argument
        """Initialize CSV handler instead of API handler.
        
        Args:
            cache_dir: Ignored. Kept for API compatibility with original ErgastAPI.
        """
        # cache_dir is intentionally unused - kept for backward compatibility
        super().__init__()
